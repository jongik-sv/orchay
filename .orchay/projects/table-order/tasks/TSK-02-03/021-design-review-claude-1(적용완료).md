...
| 이슈 | 우선순위 | 판단 | 사유 |
|------|----------|------|------|
| #1 자동화 테스트 스크립트 추가 | P3 | ✅ 적용 | 테스트 방법 개선은 검증 Task 범위 내, vitest + socket.io-client 테스트 구성 추가 |
| #2 NFR 측정 방법 명시 | P4 | ✅ 적용 | 성능 기준 검증에 필요, 각 NFR에 측정 방법 컬럼 추가 |
| #3 Node.js 기반 테스트 클라이언트 | P4 | 📝 조정 적용 | 이슈 #1 자동화 테스트로 통합 처리 (vitest 환경에서 Node.js 기반 실행) |
| #4 테스트 결과 템플릿 빈 상태 | P5 | ⏸️ 보류 | 리뷰에서도 "현재 상태 유지 OK" 명시, 실행 시 채워지는 구조 |

### 수정된 문서

| 문서 | 수정 내용 |
|------|----------|
| 010-design.md | - 섹션 2.2: NFR 테이블에 측정 방법 컬럼 추가<br>- 섹션 4.2: vitest + socket.io-client 테스트 도구 추가<br>- 섹션 4.3: 자동화 테스트 구성 신규 섹션 추가 |
